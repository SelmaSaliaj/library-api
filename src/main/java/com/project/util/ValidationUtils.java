...
public class ValidationUtils {
    ...
    /*public static void checkIfLocationWithGivenShelfNameAlreadyExists(LocationRequest request,
                                                                      LocationRepository repository){
        if((repository.findByShelfName(request.getNameOfTheShelf())) != null){
            throw new ResourceAlreadyExistsException("Location with shelf name: " + request.getNameOfTheShelf() +
                    " already exists.");
        }
    }*/
    ...
    /*
    public static void checkIfLocationWithGivenShelfNameAlreadyExists(Integer id, LocationRequest request,
                                                                      LocationRepository repository){
        LocationDTO location = LocationMapper.toDTO(repository.findByShelfName(request.getNameOfTheShelf()));
        if(location != null) {
            if (!Objects.equals(location.getId(), id)) {
                throw new ResourceAlreadyExistsException("Location with shelf name: "
                            + request.getNameOfTheShelf() + " already exists with another id: "
    ...
    public static void turnAllReservationsAndBookReservationsDeletedValuesAsRequested(ReservationEntity reservation,
        ...
        if(requestedValue){
            ...
            reservationRepository.restore(reservation);
        }
    }
        ...
        if (bookReservations != null) {
            for (BookReservationEntity bookReservation: bookReservations) {
                ...
                if(deletedValue){
                    ...
                    bookReservationRepository.restore(bookReservation);
                }
            }
        }
    }
    ...
    public static void performRestoreIfTheConditionsAreMet(Integer id, ReservationRepository reservationRepository,
                                                           BookReservationRepository bookReservationRepository){
        List<ReservationEntity> reservations = reservationRepository.findReservationsByReaderId(id);
        if (reservations != null){
            turnAllReservationsAndBookReservationsDeletedValuesAsRequested(reservations,reservationRepository, bookReservationRepository,false);
        }
    }
